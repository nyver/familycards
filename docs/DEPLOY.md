...
Пошаговое развёртывание сервера (`server/`) на чистом VPS с Caddy в роли TLS-терминатора. Мобильный клиент (`mobile/`) в текущий момент не реализован — этот документ описывает только сервер; см. `openspec/changes/add-family-card-wallet/tasks.md` для статуса остальных вех.
